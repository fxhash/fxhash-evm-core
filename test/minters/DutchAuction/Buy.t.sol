...
contract Buy is DutchAuctionTest {
    ...
    function test_RevertsWhen_InvalidPayment() public {
        ...
        dutchAuction.buy{value: price - 1}(fxGenArtProxy, reserveId, quantity, alice, alice);
    }
    ...
    function test_WhenLastStep() public {
        ...
        dutchAuction.buy{value: 0.5 ether}(fxGenArtProxy, reserveId, quantity, alice, alice);
    }

    function test_WhenEnded_PurchasePriceStaysAtLastStep() public {
        vm.warp(uint256(RESERVE_END_TIME) + 1);
        dutchAuction.buy{value: 0.5 ether}(fxGenArtProxy, reserveId, quantity, alice, alice);
    }
    ...
    function test_RevertsWhen_InvalidToken() public {
        ...
        dutchAuction.buy{value: price}(address(0), reserveId, 1, alice, alice);
    }

    function test_RevertsWhen_AddressZero() public {
        vm.expectRevert(ADDRESS_ZERO_ERROR);
        dutchAuction.buy{value: price}(fxGenArtProxy, reserveId, quantity, address(0), address(0));
    }
    ...
    function test_RevertsWhen_SoldOut() public {
        quantity = 1;
        dutchAuction.buy{value: price * MINTER_ALLOCATION}(fxGenArtProxy, reserveId, MINTER_ALLOCATION, alice, alice);
        vm.expectRevert(INVALID_AMOUNT_ERROR);
        dutchAuction.buy{value: price * quantity}(fxGenArtProxy, reserveId, quantity, alice, alice);
    }
}
