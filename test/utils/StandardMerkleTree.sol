 ...
 * @author dmfxyz (modified from https://github.com/dmfxyz/murky/blob/main/src/common/MurkyBase.sol)
 * @notice Utility contract for generating merkle roots and verifying proofs onchain
 * @dev Intended to be compatible with https://github.com/OpenZeppelin/merkle-tree
 */
contract StandardMerkleTree {
    ...
    function getProof(bytes32[] memory _data, uint256 _node) public pure returns (bytes32[] memory proof) {
        ...
        // node: max array size is 2**256-1. Largest index in the array will be 1 less than that.
    ...
    function getRoot(bytes32[] memory _data) public pure returns (bytes32) {
        ...
        while (_data.length > 1) _data = _hashLevel(_data);
        return _data[0];
    }
     ...
     */
    function _hashLevel(bytes32[] memory _data) private pure returns (bytes32[] memory result) {
